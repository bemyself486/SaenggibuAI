    ...
    with st.container(border=True):
        ...
        if st.button("🚀 전체 학생 행특 초안 생성하기", use_container_width=True):
            ...
            try:
                ...
                # 429 에러(한도 초과)를 감지하면 친절한 안내문과 블로그 링크 출력
                if "429" in error_msg or "quota" in error_msg or "rate limit" in error_msg:
                    st.error(
                        "🚨 **서버의 일일 무료 사용량이 초과되어 잠시 멈췄습니다!**\n\n"
                        "왼쪽 사이드바의 **[🛠️ 비상용 고급 설정]** 칸에 선생님의 **개인 API 키**를 입력하시면 "
                        "지금 바로 이어서 정상 작동합니다. (새로고침하지 마시고 키만 입력 후 생성 버튼을 다시 눌러주세요!)\n\n"
                        "👉 **[개인 API 키 1분 만에 무료 발급받는 방법 (블로그 안내 바로가기)](https://blog.naver.com/namjuni-lab/224325295626)**"
                    )
                else:
