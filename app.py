...
st.info("학생 수와 학년군을 설정하고 키워드를 입력하면, 전체 학급 학생의 행특을 한 번에 생성합니다. (비용: 완전 무료)")
...
def get_system_prompt(grade_group):
    return f"""
    당신은 대한민국 초등학교 베테랑 교사이자, '교육부 학교생활기록부 기재요령'을 완벽히 숙지한 전문가입니다.
    ...
    4. 문체 및 어조:
       - 철저하게 객관적인 관찰자 시점을 유지하고, 과도한 미사여구나 감정적인 표현(예: 천사 같음, 너무 예쁨 등)은 철저히 배제합니다.
    ...
    5. 긍정적 순화(우회) 원칙: 부정적인 키워드(예: 산만함, 고집, 다툼, 소극적)가 입력되더라도, 이를 학생의 성장 가능성과 교사의 발전적인 시선이 담긴 긍정적 표현으로 변환하십시오. (예: 산만함 -> 다양한 분야에 호기심이 많고 에너지가 넘치며, 주의 집중력을 기르기 위해 꾸준히 노력 중임.)

    [내용 구성 로직 (STRUCTURE)]
    생성되는 텍스트는 반드시 다음 4단계의 구조가 '줄바꿈 없는 하나의 자연스러운 문단'으로 이어지도록 작성하세요. 문단 내에 [학습 태도] 같은 대괄호나 기호는 절대 넣지 말고 문장으로만 유기적으로 연결하세요.
...
if st.button("🚀 전체 학생 행특 생성하기"):
    ...
    my_bar.progress(50, text="⚡ AI가 생기부 기재요령을 검토하며 일괄 작성 중입니다. (창을 닫거나 새로고침하지 말아주세요)")
